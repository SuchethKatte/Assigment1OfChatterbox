...
private:
    ...
    void ClientUpdate(std::string &msg){
        for(auto conn:peers){
            m_pInterface->SendMessageToConnection( conn, msg.c_str(), (uint32)strlen(msg.c_str()), k_nSteamNetworkingSend_Reliable, nullptr );
        }
    }
...
int main( int argc, const char *argv[] )
{
	...
	else
	{
		ChatServer server;
        for(int i=1;i<argc;i++){
            if(!strcmp(argv[i],"server") || !strcmp(argv[i],"--port"))
                continue;
            if(argv[i][0]=='-')
                continue;

            SteamNetworkingIPAddr other;
            other.Clear();
            if(other.ParseString(argv[i])){
                auto h = SteamNetworkingSockets()->ConnectByIPAddress(
                    other,0,nullptr
                );
                server.peers.push_back(h);
            }
        }
