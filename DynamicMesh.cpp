...
static std::string AddrToString(const SteamNetworkingIPAddr &addr)
{
...
bool AddrEqualStr(const SteamNetworkingIPAddr &a,
                  const SteamNetworkingIPAddr &b)
{
    return AddrToString(a) == AddrToString(b);
}
...
private:
    ...
    struct ServerInfo {
        ...
        HSteamNetConnection conn; // connection TO that server
    };
	...
	void OnSteamNetConnectionStatusChanged( SteamNetConnectionStatusChangedCallback_t *pInfo )
	{
		...
		switch ( pInfo->m_info.m_eState )
		{
			...
			case k_ESteamNetworkingConnectionState_Connecting:
			{
                ...
                // 1. Send existing servers to NEW server
                SendStringToClient(pInfo->m_hConn, "SERVER_LIST");
                for (auto &s : servers) {
                    SendStringToClient(pInfo->m_hConn, AddrToString(s.addr).c_str());
                }
                SendStringToClient(pInfo->m_hConn, "END");
	...
	static MeshLoader *s_pCallbackInstance;
...
MeshLoader *MeshLoader::s_pCallbackInstance = nullptr;
...
MeshState meshState = MeshState::Idle;
...
{
public:
	void Run(const SteamNetworkingIPAddr &meshLoaderAddr, uint16 nPort)
    {
        m_pInterface = SteamNetworkingSockets();
        ...
        m_hListenSock =
            m_pInterface->CreateListenSocketIP(serverLocalAddr, 1, &opt);
        ...
        m_hPollGroup = m_pInterface->CreatePollGroup();
        if (m_hPollGroup == k_HSteamNetPollGroup_Invalid)
            FatalError("Failed to create poll group");

        Printf("Server listening on port %d\n", nPort);

        // 2. Save listen addr (port known, IP filled later via HELLO)
        myListenAddr = serverLocalAddr;

        // 3. Connect to MeshLoader
        HSteamNetConnection meshConn =
            m_pInterface->ConnectByIPAddress(meshLoaderAddr, 0, nullptr);

        if (meshConn == k_HSteamNetConnection_Invalid)
            FatalError("Failed to connect to meshloader");

        Printf("Connecting to meshloader %s\n",
            AddrToString(meshLoaderAddr).c_str());
    ...
    void ConnectToMeshServer(const SteamNetworkingIPAddr &meshAddr)
    {
        m_hMeshConn = m_pInterface->ConnectByIPAddress(meshAddr, 0, nullptr);
    }

private:
    HSteamNetConnection m_hMeshConn;
	HSteamListenSocket m_hListenSock;
	HSteamNetPollGroup m_hPollGroup;
	ISteamNetworkingSockets *m_pInterface;
    SteamNetworkingIPAddr myListenAddr;


    bool IsMeshServer(HSteamNetConnection conn) const
    {
        return conn == m_hMeshConn;
    }
    ...
    std::vector<PeerServer> peers;

	std::map< HSteamNetConnection, Client_t > m_mapClients;
    std::map<std::string,std::string> KeyValue;

    
    void ConnectToPeerServer(const SteamNetworkingIPAddr &addr)
    {
        // Prevent self-connect
        if (AddrEqualStr(addr,myListenAddr)) return;

        for (auto &p : peers)
            if (AddrEqualStr(addr, p.addr)) return;

        HSteamNetConnection c =
            m_pInterface->ConnectByIPAddress(addr, 0, nullptr);

        peers.push_back({addr, c});
    }

    void RemovePeer(const char *addrStr)
    {
        SteamNetworkingIPAddr addr;
        if (!addr.ParseString(addrStr))
        {
            Printf("REMOVE_SERVER: invalid address %s\n", addrStr);
            return;
        }

        for (auto it = peers.begin(); it != peers.end(); ++it)
        {
            if(AddrEqualStr(it->addr,addr))
            {
                Printf("Removing peer %s\n", AddrToString(it->addr).c_str());
    ...
    void ClientUpdate(std::string &msg){
        for(auto conn:peers){
            m_pInterface->SendMessageToConnection( conn.conn, msg.c_str(), (uint32)strlen(msg.c_str()), k_nSteamNetworkingSend_Reliable, nullptr );
        }
    }
	...
	void PollIncomingMessages()
	{
		...
		while ( !g_bQuit )
		{
			...
			// Check for known commands.  None of this example code is secure or robust.
			// Don't write a real server like this, please.
            if (IsMeshServer(pIncomingMsg->m_conn))
            {
                if (strncmp(cmd, "SERVER_LIST", 11) == 0)
                {
                    meshState = MeshState::ReceivingList;
                    continue;
                }

                if (meshState == MeshState::ReceivingList)
                {
                    if (strcmp(cmd, "END") == 0)
                    {
                        meshState = MeshState::Idle;
                        continue;
                    }

                    SteamNetworkingIPAddr addr;
                    addr.ParseString(cmd);
                    ConnectToPeerServer(addr);
                    continue;
                }

                if (strncmp(cmd, "NEW_SERVER ", 11) == 0)
                {
                    SteamNetworkingIPAddr addr;
                    addr.ParseString(cmd + 11);
                    ConnectToPeerServer(addr);
                    continue;
                }

                if (strncmp(cmd, "REMOVE_SERVER ", 14) == 0)
                {
                    RemovePeer(cmd + 14);
                    continue;
                }
            ...
            if(strncmp(cmd,"1 ",2)==0){
                //UPDATE FROM SERVER, No need to Broadcast
                const char * str=cmd+2;
                if(strncmp(str,"SET ",4)==0){
                    std::stringstream TEMP(str+4);
                    std::string key,value;
                    TEMP>>key>>value;
                    KeyValue[key]=value;
                }
                if(strncmp(str,"DELETE ",7)==0){
                    std::stringstream TEMP(str+7);
                    std::string key;
                    TEMP>>key;
                    KeyValue.erase(key);
                }
            ...
            if(strncmp(cmd,"/GET ",5)==0){
                ...
                if(KeyValue.find(Key)==KeyValue.end()){
                    SendStringToClient(itClient->first,"KEY NOT FOUND");
                }
                else{
                    SendStringToClient(itClient->first,(KeyValue[Key]).c_str());
                }
            }
            if(strncmp(cmd,"/SET ",5)==0){
                const char *str=cmd+5;
                while(isspace(*str)) str++;
                std::stringstream TEMP(str);
                std::string Key,Val;
                TEMP>>Key>>Val;
                KeyValue[Key]=Val;
                std::string ToSend="1 SET ";
                ToSend+=Key;
                ToSend+=" ";
                ToSend+=Val;
                ClientUpdate(ToSend);
            ...
            if(strncmp(cmd,"/DELETE ",8)==0){
                const char *str=cmd+8;
                while(isspace(*str)) str++;
                std::stringstream TEMP(str);
                std::string Key;
                TEMP>>Key;
                KeyValue.erase(Key);
                std::string ToSend="1 DELETE ";
                ToSend+=Key;
                ClientUpdate(ToSend);
            }   
	...
	void OnSteamNetConnectionStatusChanged( SteamNetConnectionStatusChangedCallback_t *pInfo )
	{
		...
		switch ( pInfo->m_info.m_eState )
		{
			...
			case k_ESteamNetworkingConnectionState_Connected:
				// We will get a callback immediately after accepting the connection.
				// Since we are the server, we can ignore this, it's not news to us.
                if (pInfo->m_hConn == m_hMeshConn)
                {
                    std::string hello = "HELLO " + AddrToString(myListenAddr);
                    SendStringToClient(m_hMeshConn, hello.c_str());
                }
				break;
...
int main(int argc, const char *argv[])
{
    bool bClient = false;
    bool bServer = false;
    bool bMeshLoader = false;

    int nPort = DEFAULT_SERVER_PORT;
    SteamNetworkingIPAddr addrConnect;
    addrConnect.Clear();

    for (int i = 1; i < argc; ++i)
    {
        // ----- mode selection -----
        if (!bClient && !bServer && !bMeshLoader)
        {
        ...
        // server → meshloader address
        if ((bClient || bServer) && addrConnect.IsIPv6AllZeros())
        {
            if (!addrConnect.ParseString(argv[i]))
                FatalError("Invalid address '%s'", argv[i]);

            if (addrConnect.m_port == 0)
                addrConnect.m_port = DEFAULT_SERVER_PORT;
    ...
    // ----- validation -----
    int modes = (int)bClient + (int)bServer + (int)bMeshLoader;
    if (modes != 1)
        PrintUsageAndExit();
    ...
    if (bMeshLoader && !addrConnect.IsIPv6AllZeros())
        PrintUsageAndExit();
    ...
    {
        ChatClient client;
        client.Run(addrConnect);
    }
    else // server
    {
        ChatServer server;
        server.Run(addrConnect, (uint16)nPort); // mesh addr + listen port
    }
